...
class OAuthApplication:
    _ENDPOINT_URL = "https://osu.ppy.sh"
    _BASE_URL = f"{_ENDPOINT_URL}/api/v2"
    _OAUTH_URL = f"{_ENDPOINT_URL}/oauth"
    _RAW_URL = f"{_ENDPOINT_URL}/osu"
    _FAILURE_KEYS = {"error", "authentication"}
    _DEBUG_STRING = "{0}: {1}"

    def __init__(self, client_id: int, client_secret: str, redirect_uri: str, scopes: str) -> None:
    ...
    def from_config(cls, config: data.ApplicationConfig.OAuthConfig) -> OAuthApplication:
        return cls(config.client_id, config.client_secret, config.redirect_uri, config.scopes)

    @classmethod
    def get_raw_beatmap(cls, beatmap: int) -> models.RawBeatmapContainer | None:
        """
        Endpoint to retrieve ``.osu`` difficulty files
        :param beatmap: ID of the beatmap
        """
        request = requests.get(
            url=f"{cls._RAW_URL}/{beatmap}"
        )
