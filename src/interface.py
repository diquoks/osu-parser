...
class Application(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.parsing_settings_ruleset_combobox_str = ctk.StringVar(value=self._strings.localisable.parsing_settings_ruleset)
        self.parsing_settings_start_button_str = ctk.StringVar(value=self._strings.localisable.parsing_settings_start)
        ...
        self.geometry("550x300" if not self._registry.window.dimensions else f"550x300+{"+".join(self._registry.window.dimensions.split("+")[1:])}" if self._registry.window.state == "zoomed" else self._registry.window.dimensions)
        self.minsize(550, 300)
        self.resizable(width=True, height=True)
        self.protocol("WM_DELETE_WINDOW", lambda: self._save_window_settings(close_window=True))
        self.after_idle(lambda i: self.state(i), "withdrawn" if self._registry.window.state == "normal" or not self._registry.window.state else self._registry.window.state)
        ...
        # Parsing Tab
        self.main_parsing_tab = self.main_tabview.add(name=self._strings.localisable.main_parsing)
        ...
        self.parsing_settings_id_entry = ctk.CTkEntry(master=self.parsing_settings_frame, placeholder_text=self._strings.localisable.parsing_settings_id, validate="key", validatecommand=self._id_entry_validation)
        self.parsing_settings_id_entry.pack(side=ctk.TOP, padx=10, pady=15)
        self.parsing_settings_ruleset_combobox = ctk.CTkComboBox(master=self.parsing_settings_frame, variable=self.parsing_settings_ruleset_combobox_str, values=models.RulesetsUtils.rulesets.keys(), text_color=self._colors.entry_placeholder_text_color, state="readonly", command=self._parsing_settings_ruleset_combobox_select)
        ...
        self.parsing_score_bottom_recalculate_button = ctk.CTkButton(master=self.parsing_score_bottom_frame, text=self._strings.localisable.parsing_score_bottom_recalculate, state=ctk.DISABLED)
        self.parsing_score_bottom_recalculate_button.pack(anchor=ctk.E, side=ctk.RIGHT)
        # Calculator Tab
        self.main_calculator_tab = self.main_tabview.add(name=self._strings.localisable.main_calculator)
        self.main_tabview._segmented_button._buttons_dict[self._strings.localisable.main_calculator].configure(state=ctk.DISABLED)  # TODO
        # Settings Tab
        self.main_settings_tab = self.main_tabview.add(name=self._strings.localisable.main_settings)
        ...
        self.settings_oauth_username_label = ctk.CTkLabel(master=self.settings_oauth_frame, text=self._strings.localisable.settings_oauth_username_logged_out)
        self.settings_oauth_username_label.pack(side=ctk.LEFT)
        self.settings_oauth_login_button = ctk.CTkButton(master=self.settings_oauth_frame, text=self._strings.localisable.settings_oauth_login, command=self.oauth_login)
        ...
        self.settings_bottom_help_button = ctk.CTkButton(master=self.settings_bottom_frame, text=self._strings.localisable.settings_bottom_help, command=lambda: webbrowser.open(self._strings.url.guide), font=self._fonts.underlined)
        self.settings_bottom_help_button.pack(anchor=ctk.W, side=ctk.LEFT)
        self.settings_bottom_creator_label = ctk.CTkLabel(master=self.settings_bottom_frame, text=self._strings.localisable.settings_bottom_creator, font=self._fonts.underlined, cursor="hand2", anchor=ctk.W, height=int())
        ...
        self.settings_options_window_title_label = ctk.CTkLabel(master=self.settings_options_window_frame, text=self._strings.localisable.settings_options_window_title)
        self.settings_options_window_title_label.pack(anchor=ctk.CENTER, side=ctk.TOP, padx=10, pady=5)
        self.settings_options_window_topmost_checkbox = ctk.CTkCheckBox(master=self.settings_options_window_frame, text=self._strings.localisable.settings_options_window_topmost, variable=self.settings_options_window_topmost_checkbox_bool, command=self._settings_options_window_topmost_checkbox_select, hover_color=self._colors.progressbar_fg_color)
        self.settings_options_window_topmost_checkbox.pack(anchor=ctk.NW, side=ctk.TOP, padx=10, pady=5)
        self.settings_options_window_theme_combobox = ctk.CTkComboBox(master=self.settings_options_window_frame, variable=self.settings_options_window_theme_combobox_str, values=self._strings.localisable.settings_options_window_themes_list, state="readonly", command=self._settings_options_window_theme_combobox_select)
        ...
        self.settings_options_parsing_title_label = ctk.CTkLabel(master=self.settings_options_parsing_frame, text=self._strings.localisable.settings_options_parsing_title)
        ...
        self.settings_options_parsing_left_float_values_checkbox = ctk.CTkCheckBox(master=self.settings_options_parsing_left_frame, text=self._strings.localisable.settings_options_parsing_float_values, variable=self.settings_options_parsing_float_values_bool, command=self._settings_options_parsing_float_values_checkbox_select, hover_color=self._colors.progressbar_fg_color)
        self.settings_options_parsing_left_float_values_checkbox.pack(anchor=ctk.NW, side=ctk.TOP, padx=10, pady=5)
        self.settings_options_parsing_left_include_fails_checkbox = ctk.CTkCheckBox(master=self.settings_options_parsing_left_frame, text=self._strings.localisable.settings_options_parsing_include_fails, variable=self.settings_options_parsing_include_fails_bool, command=self._settings_options_parsing_include_fails_checkbox_select, hover_color=self._colors.progressbar_fg_color)
        ...
        self.settings_options_parsing_right_lazer_mode_checkbox = ctk.CTkCheckBox(master=self.settings_options_parsing_right_frame, text=self._strings.localisable.settings_options_parsing_lazer_mode, variable=self.settings_options_parsing_lazer_mode_bool, command=self._settings_options_parsing_lazer_mode_checkbox_select, hover_color=self._colors.progressbar_fg_color)
        ...
        if self._registry.previous.ruleset:
            try:
                self.parsing_settings_ruleset_combobox_str.set(value=models.RulesetsUtils.names[self._registry.previous.ruleset])
        ...
        try:
            self.settings_options_window_theme_combobox_str.set(value=self._strings.localisable.settings_options_window_themes_list[self._colors.available_appearance_modes.index(ctk.get_appearance_mode().lower())])
        except Exception as e:
            self._logger.info(self._strings.log.error_combobox_value, exc_info=e)
        self._logger.info(self._strings.separator.space.join((
            self._NAME,
            self._strings.localisable.settings_bottom_version_beta.format(self._config.settings.version) if self._config.settings.beta else self._config.settings.version,
    ...
    def _parsing_settings_ruleset_combobox_select(self, value: str) -> None:
        self.parsing_settings_ruleset_combobox.configure(text_color=self._colors.combobox_text_color)
        self._logger.debug(self._strings.debug.function_data.format(sys._getframe().f_code.co_name, models.RulesetsUtils.rulesets[value]))
    ...
    def _settings_options_window_theme_combobox_select(self, value: str) -> None:
        ctk.set_appearance_mode(self._colors.available_appearance_modes[self._strings.localisable.settings_options_window_themes_list.index(value)])
        self._save_window_settings()
        self._logger.debug(self._strings.debug.function_data.format(sys._getframe().f_code.co_name, self._colors.available_appearance_modes[self._strings.localisable.settings_options_window_themes_list.index(value)]))
    ...
    def parsing_thread(self, parsing_user: int, parsing_ruleset: str) -> None:
        try:
            self.parsing_score_bottom_status_label.configure(text=self._strings.localisable.parsing_score_bottom_status_player)
            try:
                user = self._oauth.get_user(user=parsing_user, ruleset=parsing_ruleset)
                self.parsing_score_top_user_label.configure(text=self._strings.localisable.parsing_score_top_user.format(user.username, user.statistics.global_rank) if user.statistics.global_rank else user.username)
                ...
                parsing_values = models.ParsingValues(pp_total=user.statistics.pp, pp_diff=float(), score_id=None, settings=self._registry.settings.refresh())
                while True:
                    self.parsing_score_bottom_status_label.configure(text=self._strings.localisable.parsing_score_bottom_status_score)
                    ...
                    if score and (parsing_values.score_id != score.id or parsing_values.settings.values != self._registry.settings.refresh().values):
                        ...
                        self.parsing_score_bottom_status_label.configure(text=self._strings.localisable.parsing_score_bottom_status_profile)
                        self._parsing_settings_progressbar_set(value=0.6)
                        user = self._oauth.get_user(user=parsing_user, ruleset=parsing_ruleset)
                        self.parsing_score_bottom_status_label.configure(text=self._strings.localisable.parsing_score_bottom_status_beatmap)
                        self._parsing_settings_progressbar_set(value=0.8)
                        beatmap = score.beatmap
                        beatmapset = score.beatmapset
                        beatmap_raw = self._oauth.get_raw_beatmap(beatmap=beatmap.id)
                        recalculation = utils.calculate_pp(score=score, beatmap_raw=beatmap_raw, lazer_mode=bool(self._registry.settings.lazer_mode))
                        ...
                        self.parsing_score_top_user_label.unbind(sequence="<ButtonPress-1>")
                        self.parsing_score_top_user_label.bind(sequence="<ButtonPress-1>", command=lambda i: webbrowser.open(self._strings.url.user_ruleset.format(parsing_user, parsing_ruleset)))
                        self.parsing_score_beatmap_label.configure(text=self._strings.localisable.parsing_score_beatmap.format(beatmapset.artist, beatmapset.title, beatmapset.creator))
                        ...
                        self.parsing_score_difficulty_label.configure(
                            text=self._strings.separator.column.join(
                                (i for i in (
                                    self._strings.localisable.parsing_score_difficulty.format(beatmap.version, beatmap.status.capitalize(), self._round_float_values(recalculation.difficulty.stars, adaptive=False)),
                                    self._strings.separator.comma.join(
                                        (i for i in (
                                            self._strings.localisable.parsing_score_pp_fc.format(self._round_float_values(recalculation.performance_fc.pp)) if not score.is_perfect_combo else None,
                                            self._strings.localisable.parsing_score_pp_ss.format(self._round_float_values(recalculation.performance_ss.pp))
                                        ) if i)
                            ...
                            text=self._strings.separator.comma.join(
                                (i for i in (
                                    self._strings.separator.space.join(i for i in (
                                        self._strings.localisable.parsing_score_pp_total.format(self._round_float_values(user.statistics.pp)),
                                        self._strings.localisable.parsing_score_pp_diff.format(self._round_float_values(parsing_values.pp_diff)) if parsing_values.pp_diff != float() else None
                                    ) if i),
                                    self._strings.separator.dash.join(i for i in (
                                        self._strings.localisable.parsing_score_pp_score.format(self._round_float_values(score.pp)),
                                        self._strings.localisable.parsing_score_pp_rank.format(score.rank_global) if score.rank_global else None,
                                    ) if i) if score.pp else None,
                                    self._strings.localisable.parsing_score_pp_weight.format(self._round_float_values(score.weight.percentage), score_weight.place, self._round_float_values(score.weight.pp)) if score.weight else None,
                                ) if i)
                            )
                        )
                        self.parsing_score_mods_label.configure(text=self._strings.localisable.parsing_score_mods.format(self._strings.separator.comma.join(score.mods.mods_string)) if score.mods.mods else str())
                        ...
                        self.parsing_score_data_stats_label.configure(text=self._strings.localisable.parsing_score_data_stats.format(self._round_float_values(score.accuracy * 100, adaptive=False), score.max_combo))
                        self.parsing_score_hits_label.configure(text=self._strings.localisable.parsing_score_hits_list[ruleset_index].format(*parsing_score_hits_values[ruleset_index]).replace(str(None), str(int())))
                        self.parsing_score_supp_hits_label.configure(text=self._strings.localisable.parsing_score_supp_hits_list[ruleset_index].format(*parsing_score_supp_hits_values[ruleset_index]).replace(str(None), str(int())))
                        # TODO
                    ...
                    self.parsing_score_bottom_status_label.configure(text=self._strings.localisable.parsing_score_bottom_status_wait)
                    for i in range(6):
            ...
            self.parsing_settings_start_button_str.set(value=self._strings.localisable.parsing_settings_start)
    ...
    def parsing_start(self) -> None:
        if not self.parsing_settings_id_entry.get().isdigit() or not self.parsing_settings_ruleset_combobox.get() or self.parsing_settings_ruleset_combobox.get() == self._strings.localisable.parsing_settings_ruleset:
            self.bell()
        else:
            self._registry.previous.update(user=int(self.parsing_settings_id_entry.get()), ruleset=models.RulesetsUtils.rulesets[self.parsing_settings_ruleset_combobox.get()])
            self.parsing_settings_progressbar.configure(mode="determinate")
            self._parsing_settings_progressbar_set()
            self._parsing_thread = threading.Thread(target=self.parsing_thread, args=[int(self.parsing_settings_id_entry.get()), models.RulesetsUtils.rulesets[self.parsing_settings_ruleset_combobox.get()]], daemon=True, name="parsingThread")
            ...
            self.parsing_settings_start_button_str.set(value=self._strings.localisable.parsing_settings_stop)
    ...
    def oauth_thread(self, login: bool = False) -> None:
        ...
        try:
            ...
            self.settings_oauth_username_label.configure(text=self._strings.localisable.settings_oauth_username_logging_in)
            self.settings_oauth_login_button.configure(state=ctk.DISABLED, text=self._strings.localisable.settings_oauth_login, command=self.oauth_login)
            if not login:
            ...
            self.settings_oauth_username_label.configure(text=self._strings.localisable.settings_oauth_username_logged_in.format(user_data.username))
            self.settings_oauth_login_button.configure(state=ctk.NORMAL, text=self._strings.localisable.settings_oauth_logout, command=self.oauth_logout)
            self.parsing_settings_start_button.configure(state=ctk.NORMAL)
        except Exception as e:
            self._logger.info(self._strings.log.error_refresh_token, exc_info=e)
            self.settings_oauth_avatar_label.configure(image=ctk.CTkImage(dark_image=self._assets.round_corners(image=self._assets.images.avatar_guest, radius=24), size=(32, 32)))
            self.settings_oauth_username_label.configure(text=self._strings.localisable.settings_oauth_username_logged_out)
    ...
    def oauth_logout(self) -> None:
        ...
        try:
            ...
            self.settings_oauth_avatar_label.configure(image=ctk.CTkImage(dark_image=self._assets.round_corners(image=self._assets.images.avatar_guest, radius=24), size=(32, 32)))
            self.settings_oauth_username_label.configure(text=self._strings.localisable.settings_oauth_username_logged_out)
            self.settings_oauth_login_button.configure(state=ctk.NORMAL, text=self._strings.localisable.settings_oauth_login, command=self.oauth_login)
    ...
    def check_for_updates_thread(self) -> None:
        if query.check_for_updates(version=self._config.settings.version, beta=self._config.settings.beta):
            self.settings_bottom_version_label.configure(text=self._strings.localisable.settings_bottom_version_update.format(self._config.settings.version))
